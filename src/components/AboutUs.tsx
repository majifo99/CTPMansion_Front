...
import { useAboutUsContent } from '../hooks/useAboutUsContent'; // Asegúrate de que la ruta sea correcta

const AboutUs = () => {
  const { content, loading, error } = useAboutUsContent(); // Usa el hook para obtener el contenido
        ...
        <div className="flex flex-col items-center justify-between py-16 lg:flex-row">
          <div className="relative z-40">
            <div className="lg:max-w-xl lg:pr-5">
              <p className="flex text-sm uppercase text-g1">{content.title}</p>
          ...
          {content.url_Image && (
            <div className="relative hidden lg:block lg:ml-32 lg:w-1/2">
              <div className="mx-auto overflow-hidden rounded-[6rem] rounded-br-none rounded-tl-none w-fit">
                <img src={content.url_Image} alt="Nosotros" className="w-full h-full object-cover" />
              </div>
            </div>
          )}
        </div>
